...
with tf.Graph().as_default():
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options,
      ...
      for step in range(FLAGS.max_iters):
        ...
        rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss, mask_ap, mask_loss, \ 
        gt_boxesnp, \
      ...
      # TODO: sampling strategy
        ...
        if step % 1 == 0: 
            print ( """iter %d: image-id:%07d, time:%.3f(sec), regular_loss: %.6f, """
                    """total-loss %.4f(%.4f, %.4f, %.6f, %.4f, %.4f, %.4f), """
                    """instances: %d, """
                    """batch:(%d|%d, %d|%d, %d|%d)""" 
                   % (step, img_id_str, duration_time, reg_lossnp, 
                      tot_loss, rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss,mask_ap, mask_loss,
                      gt_boxesnp.shape[0], 
                      rpn_batch_pos, rpn_batch, refine_batch_pos, refine_batch, mask_batch_pos, mask_batch))
