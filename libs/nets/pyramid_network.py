...
from libs import capslayer
from libs import netslayer
  ...
  with slim.arg_scope(arg_scope):
    with tf.variable_scope('pyramid'):
        ...
        with tf.variable_scope('PrimaryCaps_layer'):
            primaryCaps, activation = capslayer.layers.primaryCaps(m, filters=32, kernel_size=3, strides=2, out_caps_shape=[8, 1], padding='SAME') # return [batch_size, 10,10,32, 8,1]
        
        with tf.variable_scope('fc_reshape_Caps_layer'):
            fc_Caps, activation = capslayer.layers.fully_connected(primaryCaps, activation, num_outputs=7*7*1, out_caps_shape=[8, 1], routing_method='DynamicRouting')
            fc_Caps = tf.reshape(fc_Caps, shape=[batch_size, 7, 7,1, 8,1])
            
        with tf.variable_scope('dePrimaryCaps_layer'):
            output = capslayer.layers.dePrimaryCaps(fc_Caps, activation, num_outputs=128, kernel_size=3, strides=2) # [batch, 16, 16, ngf * 8 ] => [batch, 4, 4, ngf * 8]
            tf.layers.conv2d_transpose(output,256, kernel_size=9, strides=2);
...
def build_losses(pyramid, outputs, gt_boxes, gt_masks,
  ...
  *_lw: loss weight of rpn, refined and mask losses
  ...
  features = outputs['pyramid_feature']
...
def center_loss(features,labels,alpha, num_classes):
    len_features = features.get_shape()
